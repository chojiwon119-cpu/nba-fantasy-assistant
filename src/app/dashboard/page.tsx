...
const EMPTY_MATCHES: PlayerMatchState = { loading: false, total: 0, matched: 0, ambiguous: 0, unmatched: 0 };
...
export default function DashboardPage() {
  ...
  const [playerMatches, setPlayerMatches] = useState<PlayerMatchState>(EMPTY_MATCHES);
  ...
  useEffect(() => {
    ...
    const loadingTimer = window.setTimeout(() => {
      setPlayerMatches((current) => ({ ...current, loading: true, error: undefined }));
    }, 0);
    fetch('/api/nba/match', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ players: observedPlayers }),
      signal: controller.signal,
    })
      .then(async (response) => {
        const payload = await response.json() as { summary?: Omit<PlayerMatchState, 'loading' | 'error'>; error?: string };
        if (!response.ok || !payload.summary) throw new Error(payload.error ?? '선수 자동 연결 실패');
        setPlayerMatches({ loading: false, ...payload.summary });
      })
      .catch((error: unknown) => {
        if (error instanceof Error && error.name === 'AbortError') return;
        setPlayerMatches((current) => ({ ...current, loading: false, error: error instanceof Error ? error.message : '선수 자동 연결 실패' }));
      });
    return () => {
    ...
    <div className="playbook-shell">
      ...
      <main className="playbook-main">
        ...
        <section className="metric-grid" aria-label="리그 동기화 현황">
          ...
          <article className="metric-card"><span className="metric-icon purple"><Database size={19} /></span><div><span>NBA ID 자동 연결</span><strong>{playerMatches.loading ? '…' : playerMatches.total ? `${playerMatches.matched}/${playerMatches.total}` : '—'}</strong><small>{playerMatches.error ?? (pageCoverage.size ? `${[...pageCoverage].join(' · ')} 화면` : '수집 대기')}</small></div></article>
        </section>
        ...
        <section className="dashboard-grid">
          <article className="panel primary-panel">
            ...
            {observedPlayers.length === 0 ? (
              ...
              <div className="decision-list">
                ...
                <div className="decision-row"><span className="decision-rank">02</span><div><strong>{playerMatches.matched === playerMatches.total && playerMatches.total > 0 ? 'Yahoo 선수 자동 연결 완료' : 'Yahoo 선수 자동 연결 확인 중'}</strong><p>{playerMatches.error ?? `${playerMatches.total}명 중 ${playerMatches.matched}명을 NBA 공식 선수 ID에 연결했습니다.`}</p></div><span className={`decision-status ${playerMatches.matched === playerMatches.total && playerMatches.total > 0 ? 'ready' : 'waiting'}`}>{playerMatches.loading ? 'SYNC' : playerMatches.unmatched || playerMatches.ambiguous ? 'CHECK' : 'READY'}</span></div>
                <div className="decision-row"><span className="decision-rank">03</span><div><strong>{nbaProvider.connected ? '예측 엔진 데이터 연결 완료' : '외부 예측 데이터 연결 필요'}</strong><p>{nbaProvider.message}</p></div><span className={`decision-status ${nbaProvider.connected ? 'ready' : 'waiting'}`}>{nbaProvider.connected ? 'READY' : 'WAITING'}</span></div>
              </div>
            )}
          </article>
          ...
          <aside className="panel data-health">
            ...
            <div className="health-row"><span>NBA 스탯</span><strong className={nbaProvider.connected ? 'fresh' : 'stale'}>{nbaProvider.connected ? `${nbaProvider.provider} 연결됨` : '공식 데이터 연결 확인 필요'}</strong></div>
