...
NBA 데이터는 공급자 어댑터를 통해 서버에서만 수집됩니다. NBA.com 공식 웹 피드를
우선 검토했지만 Vercel 서버 요청이 차단되므로, 운영 기본값은 무료
[ESPN NBA](https://www.espn.com/nba/) 공개 데이터입니다. API 키나 유료 구독은 필요하지
않습니다. NBA.com 어댑터는 향후 네트워크 접근이 가능할 때 사용할 수 있도록 남겨두고,
API-Sports와 BALLDONTLIE도 선택 가능한 대체 공급자로 유지합니다.

1. `.env.example`처럼 `NBA_DATA_PROVIDER=espn-public`을 사용합니다. 기존 `api-sports` 배포 설정도 자동 전환됩니다.
...
동일한 외부 요청은 서버에서 캐시하고 진행 중 요청도 병합합니다. 공개 데이터
